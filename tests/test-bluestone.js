/**
 * Test script for Bluestone product link detection
 * 
 * This script tests the crawler's ability to find product links on Bluestone
 */
...
async function testBluestoneProductLinkDetection() {
  console.log('Starting Bluestone product link detection test...');
  ...
  try {
    // Navigate to Bluestone pendants page
    console.log('Navigating to Bluestone pendants page...');
    await page.goto('https://www.bluestone.com/pendants.html', { timeout: 30000 });
    
    // Wait for the page to load
    await page.waitForLoadState('networkidle');
    
    // Take a screenshot for reference
    await page.screenshot({ path: path.join(__dirname, 'bluestone-page.png') });
    ...
    console.log(`Found ${productLinks.length} product links on Bluestone`);
    
    // Save the product links to a file for inspection
    await fs.writeFile(
      path.join(__dirname, 'bluestone-product-links.json'),
      JSON.stringify(productLinks, null, 2)
    );
    
    // Verify that we found at least one product link
    if (productLinks.length > 0) {
      console.log('✅ Test PASSED: Found product links on Bluestone');
      ...
      if (productUrl.startsWith('/')) {
        productUrl = new URL(productUrl, 'https://www.bluestone.com').href;
      ...
      await page.screenshot({ path: path.join(__dirname, 'bluestone-product-page.png') });
      ...
      const isProductPage = currentUrl.includes('/pendant/') || 
                           currentUrl.includes('/pendants/') || 
                           currentUrl.includes('~') ||
                           currentUrl !== 'https://www.bluestone.com/pendants.html';
      ...
      console.log('❌ Test FAILED: No product links found on Bluestone');
    }
  } catch (error) {
...
// Run the test
testBluestoneProductLinkDetection().catch(console.error);
